...
template <typename T>
vector<T> step_three(vector<T> u, int L) {
    vector<T> result(L);
    T sum;
    for (int i = L; i > 0; i--) {
        sum = 0;
...
//vector<int> rsr_pp_inference(vector<int> v, const vector<vector<int>>& permutations, const vector<vector<int>>& segments, int L);
template <typename T>
vector<T> binary_rsr_pp_inference(vector<T> v, const vector<vector<int>>& permutations, const vector<vector<int>>& segments, int L) {
    ...
    for (int i = 0; i < us.size(); i++) {
        ...
        partial_result = step_three(us[i], L);
        for (int j = 0; j < L; j++) {
...
template <typename T>
vector<T> vector_rsr_pp_inference(vector<T> v, const vector<vector<int>>& permutations_pos, const vector<vector<int>>& segments_pos, const vector<vector<int>>& permutations_neg, const vector<vector<int>>& segments_neg, int L) {
    ...
    for (int i = 0; i < us[0].size(); i++) {
        // partial_result = vectorMatrixMultiply(us[i], bin_L);
        partial_result[0] = step_three(us[0][i], L);
        partial_result[1] = step_three(us[1][i], L);
        for (int j = 0; j < L; j++) {
...
template <typename T>
vector<vector<T>> rsr_pp_inference(vector<vector<T>> v, const vector<vector<int>>& permutations_pos, const vector<vector<int>>& segments_pos, const vector<vector<int>>& permutations_neg, const vector<vector<int>>& segments_neg, int L) {
    int K = permutations_pos[0].size();
    int N =  permutations_pos.size() * L;
